...
public class ParentsAndBanks {
    ...
    //write parents, kids and bank classes
        ...
        private synchronized void takeMoney(BankAccount acct, double amount) {
            synchronized (this.acct) {
                try {
                    acct.getSemaphore().acquire();
                    try {
                        System.out.println(this.name + "Wants to buy ice cream!");
                        String msg1;
                        acct.withdraw(amount);
                        String amtString = NumberFormat.getCurrencyInstance().format(new BigDecimal(Double.toString(amount)));
                        msg1 = this.name + " took " + amtString + " in ice cream money. Balance is " + acct.getBalance();
    ...
    public class BankAccount extends Thread {
        ...
        public synchronized void deposit(double amount) {
            // the add method returns a BigDecimal with the same scale as the value of this
    ...
    public static void main(String[] args) {
        ...
        int delay = 0;
        int interval = 1000; //unit is specified in scheduled execution method call

        scheduledEx.scheduleAtFixedRate(mom, delay, ThreadLocalRandom.current().nextInt(300, 3000), MILLISECONDS);
        scheduledEx.scheduleAtFixedRate(dad, delay, ThreadLocalRandom.current().nextInt(300, 3000), MILLISECONDS);


        // execute the kids

        for (int i = 0; i < numKids; i++) {
            scheduledEx.scheduleAtFixedRate(parentsAndBanks.new Kid(bank, "Kid" + Integer.toString(i)), delay, ThreadLocalRandom.current().nextInt(300, 3000), MILLISECONDS);
        }
...
//        timer.schedule(shutdownTask,10000);
